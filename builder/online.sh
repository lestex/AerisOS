...
# Packages needed for the configurator (installer will fetch its own)
arch_packages=(git gum openssl iw jq)

# We build our iso here
cache_dir=$(realpath --canonicalize-missing ~/.cache/aerisos/iso_$(date +%Y-%m-%d))
mkdir -p $cache_dir
cd $cache_dir
...
# Fix permissions on output ISO(s)
chmod 644 /out/*.iso
#chown $(stat -c '%u:%g' /out) /out/*.iso || true
